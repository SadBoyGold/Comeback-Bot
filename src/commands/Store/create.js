...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub
            ...
            .addIntegerOption(option => option
                .setName('price')
                .setDescription('Amount paid for the order')
                .setRequired(true)
                .setMinValue(0))
            .addUserOption(option => option
                .setName('paid_to')
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        let dmSent = true;
        try {
            const customerDm = createEmbed({
                title: '🛍️ Order Created',
                description: `Your Comeback Towny order has been created successfully!\n\n**Order Number:** \`${orderId}\``,
                color: 'primary',
            }).addFields(
                { name: 'Item', value: item, inline: true },
                { name: 'Price', value: `€${price.toLocaleString()}`, inline: true },
                { name: 'Paid To', value: `<@${paidTo.id}>`, inline: true },
                { name: 'Order Date', value: `<t:${Math.floor(Date.parse(createdAt) / 1000)}:F>`, inline: false },
            ).setFooter({ text: `Keep this order number: ${orderId}` });

            await customer.send({ embeds: [customerDm] });
        } catch (error) {
            dmSent = false;
            console.warn(`[Store] Could not DM customer for order ${orderId}:`, error?.message || error);
        }

        const embed = createEmbed({
            title: 'Order Created',
            description: `Order **${orderId}** has been saved successfully.${dmSent ? `\n\n📩 The customer was sent their order number by DM.` : `\n\n⚠️ The customer could not be DMed (their DMs may be closed).`}`,
            color: 'success',
        }).addFields(
