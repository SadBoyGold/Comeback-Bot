...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub
            ...
            .addStringOption(option => option
                .setName('item')
                .setDescription('Seleziona cosa ha acquistato il cliente')
                .setRequired(true)
                .addChoices(
                    { name: 'CHUNK DELLA CITTÀ', value: 'chunk_della_citta' },
                    { name: 'Pacchetto Starter', value: 'pacchetto_starter' },
                    { name: 'Pacchetto Starter Extra', value: 'pacchetto_starter_extra' },
                    { name: 'Pacchetto Starter Premium', value: 'pacchetto_starter_premium' },
                    { name: 'Pacchetto Guerriero', value: 'pacchetto_guerriero' },
                    { name: 'Pacchetto Minatore', value: 'pacchetto_minatore' },
                    { name: 'Pacchetto Agricoltore', value: 'pacchetto_agricoltore' },
                    { name: 'Pacchetto Costruttore', value: 'pacchetto_costruttore' },
                ))
            .addIntegerOption(option => option
                .setName('numero_chunk')
                .setDescription('Numero di chunk (compilalo solo per CHUNK DELLA CITTÀ)')
                .setRequired(false)
                .setMinValue(1)
                .setMaxValue(100000))
            .addIntegerOption(option => option
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        const chunkQuantity = interaction.options.getInteger('numero_chunk');
        const itemNames = {
            chunk_della_citta: 'CHUNK DELLA CITTÀ',
            pacchetto_starter: 'Pacchetto Starter',
            pacchetto_starter_extra: 'Pacchetto Starter Extra',
            pacchetto_starter_premium: 'Pacchetto Starter Premium',
            pacchetto_guerriero: 'Pacchetto Guerriero',
            pacchetto_minatore: 'Pacchetto Minatore',
            pacchetto_agricoltore: 'Pacchetto Agricoltore',
            pacchetto_costruttore: 'Pacchetto Costruttore',
        };
        const item = itemNames[itemValue];
        const price = interaction.options.getInteger('price', true);
        const paidTo = interaction.options.getUser('paid_to');
        const note = interaction.options.getString('note')?.trim() || null;

        if (itemValue === 'chunk_della_citta' && !chunkQuantity) {
            throw createError('Numero di chunk mancante', ErrorTypes.VALIDATION, 'Per **CHUNK DELLA CITTÀ** devi specificare il numero di chunk nel campo **numero_chunk**.');
        }

        const quantity = itemValue === 'chunk_della_citta' ? chunkQuantity : 1;
        const displayItem = itemValue === 'chunk_della_citta'
            ? `${item} — ${quantity} chunk`
            : item;
        ...
        try {
            await customer.send({
                embeds: [createEmbed({
                    ...
                    { name: 'Articolo', value: displayItem, inline: true },
